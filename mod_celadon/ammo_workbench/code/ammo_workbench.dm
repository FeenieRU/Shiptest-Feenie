...
/obj/machinery/ammo_workbench
	...
	icon = 'mod_celadon/_storge_icons/icons/obj/ammo_workbench.dmi'
...
/obj/machinery/ammo_workbench/ui_act(action, params)
	...
	switch(action)
		...
		if("Release")
			...
			var/stored_amount = CEILING(amount / 100, 0.1)
...
/obj/machinery/ammo_workbench/RefreshParts()
	...
	for(var/obj/item/stock_parts/micro_laser/new_laser in component_parts)
		time_efficiency -= new_laser.rating * 2 // there's two lasers
	...
	for(var/obj/item/stock_parts/manipulator/new_manipulator in component_parts)
		efficiency -= new_manipulator.rating * 0.1 // there's two lasers

	creation_efficiency = max(0, efficiency) // with paired lasers of appropriate tier, progression is 1.2 -> 1 -> 0.8 -> 0.6
	...
	for(var/obj/item/stock_parts/matter_bin/new_matter_bin in component_parts)
		mat_capacity += new_matter_bin.rating * (40 * 100)
...
/datum/material/proc/Initialize(_id, ...)
	...
	if(texture_layer_icon_state)
		cached_texture_filter_icon = icon('mod_celadon/_storge_icons/icons/turf/composite.dmi', texture_layer_icon_state)
